...
validParams<ImproveElements>()
{
  ...
  params.addClassDescription("Improves the interior node distribution in a mesh to avoid elements with pathological aspect ratios etc.");
  params.addParam<unsigned int>("iterations", 1, "Iterate this many times over all nodes to balance positions");
  params.addParam<Real>("relaxation_factor", 1.0, "How far should the node be moved toward the center of mass of its neighboring nodes ");
  return params;
}
...
ImproveElements::modify()
{
  ...
  const std::map<ElemType, std::vector<std::vector<unsigned int>>> connections = {
      ...
      {QUAD9, {{7, 4, 8}, {4, 5, 8}, {5, 6, 8}, {6, 7, 8}, {0, 1, 5, 7, 8}, {1, 2, 4, 6, 8}, {2, 3, 5, 7, 8}, {0, 3, 4, 6, 8}, {0, 1, 2, 3, 4, 5, 6, 7}}},
      {TRI3, {{1, 2}, {0, 2}, {0, 1}}},
