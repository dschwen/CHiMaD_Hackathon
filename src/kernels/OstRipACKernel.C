...
registerMooseObject("ChimadHackathonApp", OstRipACKernel);

template <>
InputParameters
validParams<OstRipACKernel>()
{
  ...
  params.addParam<MaterialPropertyName>(
      "gamma", 1.0, "Material Proprty/co-effieient for free energy term");
...
OstRipACKernel::OstRipACKernel(const InputParameters & parameters)
  : ACBulk<Real>(parameters),
    ...
    _ncrys(coupledComponents("v")) // determine number of grains from the number of names passed in.
                                   // Note this is the actual number -1
{
...
OstRipACKernel::computeDFDOP(PFFunctionType type)
{
  ...
  {
    case Residual:
      return -_gamma[_qp] * (_c[_qp] - _conc_alpha) * (_c[_qp] - _conc_alpha) * _u[_qp] +
             _beta * _u[_qp] * _u[_qp] * _u[_qp] + _epsilon * _u[_qp] * SumEtaj;

    case Jacobian:
      return (-_gamma[_qp] * (_c[_qp] - _conc_alpha) * (_c[_qp] - _conc_alpha) +
              3.0 * _beta * _u[_qp] * _u[_qp] + _epsilon * SumEtaj) *
             _phi[_j][_qp];
  }
